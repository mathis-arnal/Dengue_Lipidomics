##########################################################################################
# LIPIDOMICS ANALYSIS: POST-HOC HEALTHY vs DISEASED, ALL-PATIENTS COHORT (MILD + SEVERE
# POOLED, NOT RESTRICTED TO THE 40-PATIENT COHORT MATCHED TO THE PLASMA STUDY)
##########################################################################################
#
# COMPLETE ANALYSIS WORKFLOW (identical to PBMC_06_posthoc_healthy_vs_diseased.R):
...
#
# PBMC_06 restricts to the canonical 40-patient cohort (20 healthy + 20 sick) matched
# 1:1 to the plasma study. This script re-derives the same healthy-vs-diseased pooling
# from the larger, PBMC-only 63-patient cohort instead (group_information_table_
# healthy_vs_sick_patients_D0_all.tsv: 20 healthy + 22 mild + 21 severe), matching
# against the same PBMC abundance file. The earlier quick DE-only check
# (PBMC_06c_all_patients_check.R) found FA 18:1 and PC 32:1 reach FDR significance
# under this larger cohort (57 matched: 15 healthy, 42 diseased) where they don't at
# n=34 -- this script re-runs the FULL pipeline (not just the t-test) on that same
# cohort so the result can be reported alongside plots/enrichment like every other
# comparison in this study, not just a bare DE table.
#
# Caveat carried over from the earlier check: the added power comes entirely from the
# diseased side (healthy n did not grow, since the same 5 healthy patients still lack
# PBMC data), so this is an unbalanced (15 vs 42) comparison on a cohort that was not
# curated to match the plasma study -- not a strict apples-to-apples replication of
# PBMC_06.
##########################################################################################
...
base_dir <- "E:/Dengue_lipidomics/analysis/PBMC/PostHoc_Healthy_vs_Diseased_AllPatients/D0"

cat("\n========================================\n")
cat("POST-HOC ANALYSIS: HEALTHY vs DISEASED, ALL-PATIENTS COHORT\n")
cat("========================================\n")
...
# Derive a healthy-vs-diseased group-info table from the ALL (63-patient) cohort,
# pooling mild+severe into a single "diseased" label.
cat("Deriving healthy-vs-diseased group information from the ALL (63-patient) cohort...\n")
...
cat("Running t-test: Healthy vs Diseased (all-patients cohort)...\n")
...
if (!is.null(desp_df)) {
  ...
  cat("  Up-regulated in Diseased:", sum(desp_df$padj < 0.05 & desp_df$log2FC > 0, na.rm = TRUE), "\n")
  cat("  Down-regulated in Diseased:", sum(desp_df$padj < 0.05 & desp_df$log2FC < 0, na.rm = TRUE), "\n")
...
# LipidSigR's internal .deSig_data(sigN=3)) -- wrapped in tryCatch() so a sparse-
# significant-hit comparison degrades gracefully instead of halting the whole script.
cat("Running PLS-DA...\n")
            ...
            if (lipid_name %in% rownames(abundance_data)) {
              ...
              if (all(group_counts >= 2)) {
                ...
                cat("    Warning: Not enough samples for", lipid_name,
                    "(healthy:", group_counts["healthy"], ", diseased:", group_counts["diseased"], ")\n")
              }
            } else {
              failed_count <- failed_count + 1
...
cat("\n========================================\n")
cat("POST-HOC ANALYSIS HEALTHY vs DISEASED (ALL-PATIENTS COHORT) COMPLETED!\n")
cat("========================================\n")
