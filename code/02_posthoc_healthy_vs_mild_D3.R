##########################################################################################
# LIPIDOMICS ANALYSIS: POST-HOC HEALTHY vs MILD
##########################################################################################
#
...
#
# This script performs pairwise comparison: Healthy vs Mild
##########################################################################################
...
base_dir <- "/Users/loictalignani/research/project/lipidomics/analysis/PostHoc_Healthy_vs_Mild/D3"

cat("\n========================================\n")
cat("POST-HOC ANALYSIS: HEALTHY vs MILD\n")
cat("========================================\n")
...
group_info <- read_tsv("data/lipidsig_datasets/healthy_vs_sick_patients/group_information_table_healthy_vs_MILD_D3.tsv",
...
abundance <- read_tsv("data/lipidsig_datasets/healthy_vs_sick_patients/healthy_MILD_lipidomics_D3.tsv",
  show_col_types = FALSE
)
...
cat("Running t-test: Healthy vs Mild...\n")
...
if (!is.null(desp_df)) {
  ...
  cat("  Up-regulated in Mild:", sum(desp_df$padj < 0.05 & desp_df$log2FC > 0, na.rm = TRUE), "\n")
  cat("  Down-regulated in Mild:", sum(desp_df$padj < 0.05 & desp_df$log2FC < 0, na.rm = TRUE), "\n")
            ...
            if (lipid_name %in% rownames(abundance_data)) {
              ...
              if (all(group_counts >= 2)) {
                ...
                p <- ggplot(plot_data, aes(x = group, y = abundance, fill = group)) +
                  ...
                  geom_signif(
                    comparisons = list(c("healthy", "mild")),
                  ...
                  scale_fill_manual(values = c("healthy" = "#4DAF4A", "mild" = "#E41A1C"))
                ...
                cat("    Warning: Not enough samples for", lipid_name,
                    "(healthy:", group_counts["healthy"], ", mild:", group_counts["mild"], ")\n")
              }
            } else {
              failed_count <- failed_count + 1
