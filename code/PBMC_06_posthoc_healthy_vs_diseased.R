##########################################################################################
# LIPIDOMICS ANALYSIS: POST-HOC HEALTHY vs DISEASED (MILD + SEVERE POOLED)
##########################################################################################
#
...
#
# This script performs the pairwise comparison PBMC_02/03/04 don't directly answer:
# healthy vs ALL dengue patients pooled into one "diseased" group (mild + severe
# combined), rather than healthy-vs-mild and healthy-vs-severe tested separately.
# This is a genuinely different statistical test, not just a re-summary of 02/03 --
# pooling changes power (larger, more balanced n per group) and can reveal a signal
# neither sub-comparison was powered to detect alone, or wash one out if mild and
# severe move in different directions relative to healthy. Uses the same canonical
# 40-patient cohort (20 healthy + 20 sick) as PBMC_01/02/03/04.
##########################################################################################
...
base_dir <- "E:/Dengue_lipidomics/analysis/PBMC/PostHoc_Healthy_vs_Diseased/D0"

cat("\n========================================\n")
cat("POST-HOC ANALYSIS: HEALTHY vs DISEASED (mild+severe pooled)\n")
cat("========================================\n")
...
# Derive a healthy-vs-diseased group-info table from the canonical 40-patient
# (20 healthy + 20 sick) cohort used by PBMC_01-04, pooling mild+severe into a single
# "diseased" label. Written to disk for transparency/reuse rather than kept in-memory only.
cat("Deriving healthy-vs-diseased group information from the canonical cohort...\n")
canonical_group_info <- read_tsv("data/PBMC/group_information_table_healthy_vs_sick_patients_D0.tsv", show_col_types = FALSE)
diseased_group_info <- canonical_group_info %>%
  mutate(group = ifelse(group %in% c("mild", "severe"), "diseased", group))
write_tsv(diseased_group_info, "data/PBMC/group_information_table_healthy_vs_diseased_D0.tsv")
cat("  Healthy:", sum(diseased_group_info$group == "healthy"),
    " Diseased:", sum(diseased_group_info$group == "diseased"), "\n")
...
cat("Running t-test: Healthy vs Diseased...\n")
...
cat("\n========================================\n")
cat("POST-HOC ANALYSIS HEALTHY vs DISEASED COMPLETED!\n")
cat("========================================\n")
